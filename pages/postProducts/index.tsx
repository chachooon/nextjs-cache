...
// const fetchProducts = () =>
//   axios
//     .get(`https://stg-api.hanwhalife.com/int/postProducts/menus/insurances`, {
//       headers:{
    ...
    <>
      ...
      {getData?.map((data: any, index: number) => (
        <div
          ...
          {/* <Link href={`/postProducts/${data.menuCategory}`}>{JSON.stringify(data)}</Link> */}
          <Link href={`/postProducts/id/?category=${data.menuCategory}`}>{JSON.stringify(data)}</Link>
        </div>
      ))}
       {getData?.map((data: any, index: number) => (
        <div
